...
<body>
	...
	<div class="container mt-5">
		<h2>Refill Due As Of Date</h2>
		<table class="table table-hover mt-4">

			<tbody>
				<c:forEach var="refill" items="${refill}">
					<tr>
						<th>Refill Order Id</th>
						<td>${refill.refillOrderId}</td>
					</tr>
					<tr>
						<th>Subscription Id</th>
						<th>${refill.subscriptionId}</th>
					</tr>

					<tr>
						<th>Refill Date</th>
						<th>${refill.refillDate}</th>
					</tr>
					<tr>
						<th>Next Refill Date</th>
						<th>${refill.refillDateNext}</th>
					</tr>
					<tr>
						<th>Payment Paid</th>
						<th>${refill.status}</th>
					...
					<tr>
						<th>Refill For Due</th>
						<th>${refill.visited}</th>
					</tr>
					<tr><th>Drug Details-</th></tr>
					<tr>
						<th>Drug Name</th>
						<th>Quantity</th>
					</tr>
					
					<c:forEach var="refillOrderLine"
						items="${refill.getRefillOrderLine()}">

						<tr>
							<td>${refillOrderLine.getDrug()}</td>
							<td>${refillOrderLine.getDrugQuantity()}</td>
						</tr>
					</c:forEach>
				</c:forEach>
			</tbody>
